...
typedef struct _dw1000_dev_status_t{
    ...
    uint32_t wakeup_LLDE:1;           //!< Load LDE microcode on wake up
    uint32_t wakeup_LLDO:1;           //!< Load the LDO tune value on wake up
    ...
    uint32_t sleeping:1;              //!< Indicates state of sleeping
}dw1000_dev_status_t;
...
typedef struct _dw1000_dev_control_t{
    ...
    uint32_t wakeup_LLDE:1;                 //!< Enables to load LDE microcode on wake up
    uint32_t wakeup_LLDO:1;                 //!< Enables to load LDO tune value on wake up
...
typedef struct _dw1000_extension_callback_t{
    ...
    void (* rx_timeout_cb)  (struct _dw1000_dev_instance_t *);    //!< Rceeive timeout callback
    void (* rx_error_cb)    (struct _dw1000_dev_instance_t *);    //!< Receive error callback
    void (* tx_error_cb)    (struct _dw1000_dev_instance_t *);    //!< Transmit error callback
    dw1000_extension_callbacks_t * next;                          //!< Next node in the list
...
typedef struct _dw1000_dev_instance_t{
    ...
    uint64_t txtimestamp;          //!< Transmit interface
    ...
    uint16_t sleep_mode;           //!< Device sleep mode
    uint8_t xtal_trim;             //!< Crystal trim
    uint32_t sys_cfg_reg;          //!< System config register
    uint32_t sys_ctrl_reg;         //!< System control register 
    ...
    struct os_eventq interrupt_eventq;     //!< Structure of os_eventq that has event queue 
    struct os_event interrupt_ev;          //!< Structure of os_event that tirgger interrupts 
    struct os_task interrupt_task_str;     //!< Structure of os_task that has interrupt task 
    uint8_t interrupt_task_prio;           //!< Priority of the interrupt task  
    os_stack_t interrupt_task_stack[DW1000_DEV_TASK_STACK_SZ]  //!< Stack of the interrupt task 
